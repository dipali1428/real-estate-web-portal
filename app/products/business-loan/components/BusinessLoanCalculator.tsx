...
const BusinessLoanCalculator = () => {
    ...
    useEffect(() => {
        if (chartRef.current) {
            ...
            chartRef.current.update('none'); // Update without animation for better performance
        }
    ...
    return (
        <div className="container mx-auto px-4 py-16">
            <div className="max-w-3xl mx-auto text-center mb-16 px-4">
                <h2 className="text-3xl md:text-5xl font-bold mb-6 tracking-tight bg-clip-text text-transparent bg-linear-to-r from-[#2076C7] to-[#1CADA3]">EMI Calculator</h2>
                <p className="text-lg text-gray-500 font-medium">
                    Plan your business finances better by calculating your monthly repayments instantly.
                </p>
            </div>

            <div className="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden max-w-7xl mx-auto border border-gray-100">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
                                {frequencyOptions.slice(0, 6).map((option) => (
                                    <button
                                        ...
                                        className={`py-2 sm:py-3 px-2 sm:px-4 rounded-lg border transition-all text-left ${paymentFrequency === option.value ? 'border-[#1CADA3] bg-teal-50 text-[#1CADA3]' : 'border-gray-300 hover:border-gray-400 text-gray-700'}`}
                                    >
                                        <div className="font-medium text-xs sm:text-sm">{option.label.split('(')[0]}</div>
                                        <div className="text-xs mt-0.5 opacity-75 hidden sm:block">
                                ...
                                {(() => {
                                    ...
                                    if (annualInterestRate > 1 && loanAmount > 0 && loanTermMonths > 0) {
                                        ...
                                        if (betterRate > 0) {
                                            ...
                                            const betterRatePayments = Math.ceil(loanTermMonths / 12 * frequencyMap[paymentFrequency]);
                                            const rateFactor = Math.pow(1 + periodicInterestRate, betterRatePayments);
                                            const betterEMI = (loanAmount * periodicInterestRate * rateFactor) / (rateFactor - 1);
                                            const totalSavings = (paymentAmount - betterEMI) * betterRatePayments;
