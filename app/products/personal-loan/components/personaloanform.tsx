...
export default function PersonalLoanForm({ onClose, prefilledData, selectedBank }: PersonalLoanFormProps) {
  ...
  const validate = () => {
    ...
    ["clientName", "location", "dob", "loanAmount", "deductionDetails", "companyName", "companyAddress"].forEach(f =>
      req(f, `${f.split(/(?=[A-Z])/).join(" ").replace(/^\w/, c => c.toUpperCase())} is required`)
    );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-2 sm:p-4 text-gray-700 font-sans" style={{ fontFamily: 'var(--font-geist-sans)' }}>
      <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl mx-auto h-[95vh] sm:h-[90vh] flex flex-col relative">
        <div className="flex justify-between items-center border-b px-4 sm:px-6 py-3 sm:py-4 shrink-0 bg-white rounded-t-xl">
          <h2 className="text-lg sm:text-xl font-semibold text-[#1CADA3]">
            {selectedBank ? `Personal Loan Application - ${selectedBank}` : 'Personal Loan Form'}
          </h2>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6">
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            ...
            <Field label="Deduction Details" placeholder="Salary deduction details" {...fieldProps("deductionDetails")} required />
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 rounded-xl animate-in fade-in zoom-in duration-200">
      <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-2xl text-center max-w-sm w-[90%] mx-auto">
        ...
        <p className="text-gray-600 mb-6">Your Personal Loan application has been submitted successfully.</p>
