...
export default function EducationLoanForm({ onClose, prefilledData }: EducationLoanFormProps) {
  ...
  const validate = () => {
    ...
    const isAnyDocUploaded = Object.keys(uploadedDocs).some(key => key.startsWith("student_") && uploadedDocs[key]);
    if (!isAnyDocUploaded) {
      errs.docs = "At least one student document is required";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 🚀 Bypassing backend as requested
      /*
      const payload = {
        department: "Loan",
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl mx-auto h-[95vh] sm:h-[90vh] flex flex-col relative">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6">
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            ...
            <Field label="Email ID" placeholder="Enter email" type="email" {...fProps("email")} required />
            ...
            <div className="col-span-1 md:col-span-2 mt-4">
              <h3 className="text-md font-semibold mb-3 text-[#1CADA3] border-b pb-2">Student Documents</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                {studentDocs.map(lbl => (
                  <FileUpload key={`st_${lbl}`} label={lbl} allowMultiple={!["Aadhar Card", "PAN Card"].includes(lbl)} onUpdate={(has: boolean) => handleDocUpdate("student", lbl, has)} />
                ))}
              </div>
              {errors.docs && <p className="text-red-500 text-xs mt-2 font-medium">{errors.docs}</p>}
            </div>
