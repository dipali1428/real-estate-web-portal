...
export default function MutualFundComparison() {
  ...
  // Preload 2 funds so comparison is not empty
  useEffect(() => {
    const loadDefaultFunds = async () => {
      if (selectedFunds.length > 0) return;

      try {
        const defaults = SUGGESTED_FUNDS.slice(0, 2);

        const fundPromises = defaults.map(async (fund) => {
          ...
          const yearAgoPoint =
            history.find((p: any) => p.date >= yearAgoDate) || history[0];

          const return1Y =
            ((latestNav - yearAgoPoint.nav) / yearAgoPoint.nav) * 100;
          ...
          return {
            ...
            risk: details.meta.scheme_category?.includes("Debt")
              ? "Low"
              : "High",
        ...
        console.error("Failed loading default funds", err);
      }
    };
  ...
  const chartData = useMemo(() => {
    ...
    return {
      ...
      datasets: selectedFunds.map((fund, idx) => {
        ...
        return {
          label: fund.name,
          data: fund.history.map((h: any) =>
            ((h.nav / baseNav) * 100).toFixed(2),
          ),
  ...
  return (
    <section
      className="relative bg-white rounded-3xl p-6 md:p-8 border border-gray-100/80 overflow-hidden transition-all duration-300 hover:-translate-y-1 space-y-8"
      style={{
        boxShadow:
    ...
    >
      <div className="text-center">
        <h2 className="text-2xl md:text-3xl font-bold font-sans bg-linear-to-r from-[#2076C7] to-[#1CADA3] bg-clip-text text-transparent mb-3">
      ...
      <div className="w-full mx-auto space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 w-full">
          <span className="shrink-0 text-sm font-medium text-slate-500 mr-2">
            Top Choices:
          </span>
          {SUGGESTED_FUNDS.map((fund) => (
            <button
              ...
              disabled={loadingFund !== null}
              className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all border ${
                selectedFunds.some((f) => f.id === fund.code)
        ...
        <div className="relative max-w-xl mx-auto">
          ...
          <input
            ...
            className="w-full pl-11 pr-4 py-2.5 text-sm bg-gray-50 text-black border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#1CADA3] focus:border-transparent transition-all"
          />
          {isSearching && (
            ...
            {searchResults.length > 0 && (
              ...
              >
                {searchResults.map((fund) => (
                  ...
                  >
                    <div>
                      <div className="font-medium text-slate-900 text-sm group-hover:text-[#2076C7]">
                        {fund.name}
                      </div>
      ...
      {selectedFunds.length > 0 ? (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-gray-200">
                    ...
                    {selectedFunds.map((fund) => (
                      <th key={fund.id} className="p-3 text-left min-w-[150px]">
                        <div className="flex justify-between items-start gap-2">
                          <span className="text-slate-950 font-bold text-sm line-clamp-2">
                            {fund.name}
                          </span>
                          <button
                ...
                <tbody className="text-sm">
                  <tr className="border-b border-gray-100">
                    <td className="p-3 font-semibold text-slate-700">NAV</td>
                    {selectedFunds.map((fund) => (
                      <td
                        key={fund.id}
                        className="p-3 font-bold text-slate-950 text-base"
                      >
                        ₹{fund.nav}
                      </td>
                    ))}
                  ...
                  <tr className="border-b border-gray-100">
                    ...
                    {selectedFunds.map((fund) => (
                      <td key={fund.id} className="p-3">
                        <div
                          className={`flex items-center gap-1 font-bold text-base ${parseFloat(fund.return1Y) >= 0 ? "text-emerald-600" : "text-rose-600"}`}
                        >
                          {parseFloat(fund.return1Y) >= 0 ? (
                            <TrendingUp size={14} />
                          ) : (
                  ...
                  <tr className="border-b border-gray-100">
                    ...
                    {selectedFunds.map((fund) => (
                      <td
                        key={fund.id}
                        className="p-3 font-bold text-slate-800"
                      >
                        {fund.category}
                      </td>
                    ))}
          ...
          <div className="lg:col-span-12 xl:col-span-7 space-y-4">
            ...
            <div className="h-[300px] w-full bg-white p-4 rounded-3xl border border-gray-100 shadow-sm relative">
              {chartData && (
                <Line
                  data={chartData}
                  options={{
        ...
        <div className="text-center py-20 bg-gray-50/50 rounded-3xl border border-dashed border-gray-200">
          ...
          <p className="text-slate-500 max-w-sm mx-auto font-light">
            Select a popular fund from above or use the search box to start
