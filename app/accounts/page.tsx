...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setUser(profile?.name || profile?.user?.name || "Account Profile");

    //   const rmData = await AccountService.getRelationshipManagers();
    //   setRmCount(rmData?.total || rmData?.rms?.length || 0);

    //   const leadsResponse = await AccountService.getDepartmentLeads();
