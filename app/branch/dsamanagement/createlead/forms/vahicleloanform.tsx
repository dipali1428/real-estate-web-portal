...
const DOC_REGISTRY: Record<string, { key: string; label: string; multiple: boolean }> = {
  "Vehicle Quotation": { key: "VEHICLE_QUOTATION", label: "Vehicle Quotation", multiple: false },
  "Aadhar Card": { key: "AADHAAR", label: "Aadhaar Card", multiple: false },
  ...
  "Bank Statement (1 Year / 6 Months)": { key: "BANK_STATEMENT", label: "Bank Statement", multiple: true },
  ...
  "Saving Bank Account Statement": { key: "SAVING_BANK_STMT", label: "Saving Bank Statement", multiple: true },
...
const DOC_MAP: Record<string, string[]> = {
  "Salaried Person": ["Aadhar Card", "PAN Card", "3 Months Salary Slip", "2 Years Form 16", "Bank Statement (1 Year / 6 Months)", "Address Proof", "Photograph", "Company ID Card"],
  "Self Employed": ["Aadhar Card", "PAN Card", "Udyam Registration", "Shop Act Licence", "Bank Statement (1 Year / 6 Months)", "Saving Bank Account Statement", "Address Proof", "3 Years ITR", "GST Certificate", "Last 12 Months GST Returns", "Photograph"]
};
...
export default function VehicleLoanForm({ onClose }: { onClose: () => void }) {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    ...
    loanAmount: "", vehicleType: "", vehicleCompany: "",
    employmentType: "", hasOtherLoan: "", otherLoanAmount: ""
  });
  ...
  const requiredDocsList = useMemo(() => {
    const labels = ["Vehicle Quotation", ...(DOC_MAP[form.employmentType] || [])];
    if (form.hasOtherLoan === "Yes") labels.push("Existing Loan Statement");
    return labels.map(label => DOC_REGISTRY[label]).filter(Boolean);
  }, [form.employmentType, form.hasOtherLoan]);

  const handleInputChange = (field: string, value: string) => {
    // Sanitization: Emails to Lowercase
    const processedValue = field === "email" ? value.toLowerCase() : value;
    setForm(p => ({ ...p, [field]: processedValue, ...(field === "hasOtherLoan" && value === "No" ? { otherLoanAmount: "" } : {}) }));
  ...
  const validateStep1 = () => {
    ...
    req("vehicleCompany", "Vehicle company is required");
  ...
  const handleCreateLead = async () => {
    ...
    try {
      const payload = {
        department: "Loan", product_type: "Vehicle Loan", sub_category: "Vehicle Loan",
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-2 sm:p-4 text-gray-700">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl mx-auto h-[95vh] sm:h-[90vh] flex flex-col relative overflow-hidden">
        ...
        <div className="flex justify-between items-center border-b px-4 sm:px-6 py-3 sm:py-4 bg-white">
          <div>
            <h2 className="text-lg sm:text-xl font-semibold text-[#1CADA3]">Vehicle Loan Application</h2>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-gray-50/30">
          {step === 1 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              ...
              <Field label="Type of Vehicle" type="select" options={["Two Wheeler", "Four Wheeler", "Commercial Vehicle"]} value={form.vehicleType} onChange={(v:any)=>handleInputChange("vehicleType", v)} error={errors.vehicleType} required />
              <Field label="Vehicle Company Name" placeholder="e.g. Honda, Tata" value={form.vehicleCompany} onChange={(v:any)=>handleInputChange("vehicleCompany", v)} error={errors.vehicleCompany} required />
              ...
              <div className="col-span-1 md:col-span-2 mt-2"><Field label="Employment Type" type="select" options={["Salaried Person", "Self Employed"]} value={form.employmentType} onChange={(v:any)=>handleInputChange("employmentType", v)} error={errors.employmentType} required /></div>
            </div>
          ) : (
