...
const TransactionManagement: React.FC = () => {
  ...
  return (
    <div className="flex-1 space-y-8 animate-fade-in pb-10">
      ...
      {/* --- STATS GRID (from EnquiryManagement) --- */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatsCard 
          icon={ShoppingCart}
          label="Buy Orders"
          value={transactions.filter(tx => tx.transaction_type?.toUpperCase() === 'BUY').length}
        ...
        <StatsCard 
          icon={HandCoins}
          label="Sell Orders"
          value={transactions.filter(tx => tx.transaction_type?.toUpperCase() === 'SELL').length}
