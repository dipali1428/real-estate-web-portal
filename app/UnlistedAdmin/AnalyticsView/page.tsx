...
const AnalyticsView = () => {
  ...
  return (
    /* ADJUSTED OUTER PADDING AND BACKGROUND HERE */
    <div className="flex-1 p-4 sm:p-6 bg-[#F8FAFC] min-h-screen font-sans">
      <div className="space-y-6 animate-fade-in pb-10">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                ...
                return (
                  <motion.div 
                    initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} key={idx} 
                    className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-all"
                  >
                    <div className={`w-12 h-12 bg-gradient-to-r ${item.color} rounded-2xl flex items-center justify-center text-white shadow-sm mb-4`}>
                      <Icon className="w-6 h-6" strokeWidth={1.5} />
                    ...
                    {transactions.length > 0 ? (
                      transactions.map((tx) => (
                        <tr key={tx.txn_id} className="hover:bg-gray-50/50 transition-colors">
                          <td className="px-6 py-4 font-bold text-gray-700 text-sm">#{tx.txn_id}</td>
                          <td className="px-6 py-4 font-medium text-gray-800">{tx.user_name}</td>
                          <td className="px-6 py-4 text-sm text-gray-700">{tx.asset_name}</td>
                          ...
                          <td className="px-6 py-4 text-xs text-gray-500">{formatDate(tx.timestamp)}</td>
                        </tr>
                      ))
                    ) : (
                      <tr><td colSpan={5} className="px-6 py-16 text-center text-gray-400">No transactions found</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
        ...
        {activeTab === 'alerts' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            {/* Logic for Alerts content keeps same structure */}
            {Array.isArray(highInvestmentAlerts) && highInvestmentAlerts.length > 0 ? (
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                <h4 className="text-sm font-black text-gray-800 uppercase tracking-widest mb-6 flex items-center gap-2">
                  <Flame className="w-4 h-4 text-rose-500" /> High Investment Alerts
                </h4>
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                    <tbody className="divide-y divide-gray-50">
                      {highInvestmentAlerts.map((alert) => (
                        <tr key={alert.share_id}>
              ...
              <div className="bg-white rounded-2xl p-8 text-center border border-gray-100 text-gray-400">No alerts found</div>
            )}
          </motion.div>
        )}
        ...
        {activeTab === 'filters' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              <h4 className="text-sm font-black text-gray-800 uppercase tracking-widest mb-4">Filter Shares</h4>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                <input
                  type="text" placeholder="Company Name" value={shareFilterParams.company_name}
                ...
                />
                <button onClick={applyShareFilters} className="px-4 py-2 bg-[#2076C7] text-white rounded-xl text-xs font-black uppercase">Apply</button>
              </div>
            </div>
          </motion.div>
        )}
