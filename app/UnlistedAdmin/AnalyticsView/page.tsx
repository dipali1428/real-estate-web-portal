...
import { 
  ...
  LayoutDashboard, Layers, Flame, X
} from 'lucide-react';
...
const AnalyticsView = () => {
  ...
  // Filter States - Initialize as empty arrays
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Handle High Investment Alerts - Ensure it's an array
      if (highInvData.status === 'fulfilled') {
        setHighInvestmentAlerts(Array.isArray(highInvData.value) ? highInvData.value : []);
      }
      ...
      // Handle Filter Data - Ensure they are arrays
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              { label: 'Active Alerts', val: Array.isArray(highInvestmentAlerts) ? highInvestmentAlerts.length : 0, icon: Bell, color: 'from-rose-500 to-pink-500' }
            ].map((item, idx) => {
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            ...
            <div className="mt-6 overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-gray-50">
                  {Array.isArray(shareFilters) && shareFilters.slice(0, 5).map((item, i) => (
                    <tr key={i} className="hover:bg-gray-50">
                      <td className="px-4 py-3 font-bold text-gray-800">{item.company_name}</td>
                      <td className="px-4 py-3">{formatCurrency(item.price)}</td>
                      <td className="px-4 py-3">{item.quantity?.toLocaleString()}</td>
                      <td className="px-4 py-3 font-black text-[#2076C7]">{formatCurrency(item.min_investment)}</td>
                    </tr>
                  ))}
                  {(!Array.isArray(shareFilters) || shareFilters.length === 0) && (
                    <tr>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-gray-50">
                  {Array.isArray(transactionFilters) && transactionFilters.slice(0, 5).map((item, i) => (
                    <tr key={i} className="hover:bg-gray-50">
                      <td className="px-4 py-3 font-bold text-gray-800">#{item.txn_id}</td>
                      ...
                      <td className="px-4 py-3">{item.quantity?.toLocaleString()}</td>
                      <td className="px-4 py-3 font-black">{formatCurrency(item.total_amount)}</td>
                      <td className="px-4 py-3">
                        <span className={`text-[10px] font-black uppercase px-2 py-1 rounded-full ${getStatusColor(item.status)}`}>
                          {item.status}
                        </span>
                      </td>
                    </tr>
                  ))}
                  {(!Array.isArray(transactionFilters) || transactionFilters.length === 0) && (
                    <tr>
