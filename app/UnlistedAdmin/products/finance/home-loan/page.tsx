...
const HomeLoanImportAdmin: React.FC = () => {
  ...
  const [plans, setPlans] = useState<HomeLoanPlan[]>([]);
  const [isLoadingPlans, setIsLoadingPlans] = useState(true);
  const [searchQuery, setSearchQuery] = useState("");
  const [editingPlan, setEditingPlan] = useState<HomeLoanPlan | null>(null);
  ...
  const startImport = async () => {
    ...
    setToast({ message: `Importing Home Loan plan data...`, type: 'info' });
    setLogs(prev => [...prev, `[${new Date().toLocaleTimeString()}] Starting Home Loan import of ${file.name}...`]);
    setImportResult(null);
    try {
      const response = await HomeLoanService.uploadHomeLoanPlans(file);
      ...
      const successMsg = `✓ Home Loan Plans imported successfully (${response.processed} rows processed, ${response.skipped} skipped)`;
  ...
  const handleDelete = async (id: number, name: string) => {
    ...
    try {
      await HomeLoanService.deleteHomeLoanPlan(id);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    setIsUpdating(true);
    try {
      await HomeLoanService.updateHomeLoanPlan(editingPlan.id, editingPlan);
      setToast({ message: `${editingPlan.bank_name} updated successfully`, type: 'success' });
  ...
  return (
    <div className="flex-1 space-y-6 animate-fade-in pb-10 px-4 sm:px-6">
      {/* HEADER */}
      <motion.div
        ...
        <div className="flex items-center gap-3">
          <div>
            <h2 className="text-2xl font-bold flex items-center gap-2">
              <Landmark className="w-6 h-6" /> Home Loan Portal
            </h2>
            <p className="text-sm opacity-90">Import & manage Home Loan bank plan data</p>
          </div>
        </div>
      </motion.div>
        ...
        >
          ...
          <div className="p-6">
            <div className="bg-gradient-to-br from-gray-50 to-white rounded-2xl p-8 border border-gray-100 text-center">
              ...
              <h4 className="text-xl font-bold text-gray-800 mb-2">Home Loan Template</h4>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    const sampleRows = [
                      ['State Bank of India (SBI)', 'New Purchase', '7.50% p.a.', '₹1,100 + GST', 'Lowest interest rates, Trusted bank, Quick approval', 'blue'],
                      ['HDFC Bank', 'Resale Property', '7.20% p.a.', '₹9,000 + GST', 'Fast processing, Large network, Digital documentation', 'blue']
                    ];
                    ...
                    link.href = url;
                    link.setAttribute('download', `home_loan_template.csv`);
        ...
        <div className="overflow-x-auto">
          {isLoadingPlans ? (
            ...
            <div className="flex flex-col items-center justify-center p-20 text-gray-400 text-center">
              ...
              <p className="text-xs">Import a new file to get started.</p>
            </div>
          ) : (
            ...
            >
              <div className="bg-gradient-to-r from-[#2076C7] to-[#1CADA3] p-6 text-white shrink-0">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="text-xl font-black uppercase tracking-tight">Edit Bank Plan</h3>
                    <p className="text-xs opacity-80">Modify bank rates and features</p>
                  </div>
                  <button onClick={() => setEditingPlan(null)} className="p-2 bg-white/20 hover:bg-white/30 rounded-full transition-colors">
                    <X className="w-5 h-5" />
                  </button>
                </div>
              </div>
...
export default HomeLoanImportAdmin;
