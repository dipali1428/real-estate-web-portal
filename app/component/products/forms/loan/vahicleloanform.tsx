...
const DOC_MAP: Record<string, string[]> = {
  "Salaried Person": ["Aadhar Card", "PAN Card", "3 Months Salary Slip", "2 Years Form 16", "6 Months Banking Statement", "Address Proof", "Photograph", "Company ID Card"],
  "Self Employed": ["Aadhar Card", "PAN Card", "Udyam Registration", "Shop Act Licence", "1 Year Current Bank Statement", "Saving Bank Account Statement", "Address Proof", "3 Years ITR", "GST Certificate", "Last 12 Months GST Returns", "Photograph"]
};
...
export default function VehicleLoanForm({ onClose, prefilledData }: VehicleLoanFormProps) {
  // Initialize state with prefilled data
  ...
  const requiredDocs = useMemo(() => {
    const list = ["Vehicle Quotation", ...(DOC_MAP[form.employmentType] || [])];
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl mx-auto h-[95vh] sm:h-[90vh] flex flex-col relative">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6">
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            ...
            <Field label="Type of Vehicle" type="select" options={["Two Wheeler", "Four Wheeler", "Commercial Vehicle"]} {...fProps("vehicleType")} required />
            <Field label="Vehicle Company Name" placeholder="e.g. Honda, Tata" {...fProps("vehicleCompany")} required />
            ...
            <div className="col-span-1 md:col-span-2 mt-2">
              <Field label="Employment Type" type="select" options={["Salaried Person", "Self Employed"]} {...fProps("employmentType")} required />
            </div>
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 rounded-xl animate-in fade-in zoom-in duration-200">
      <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-2xl text-center max-w-sm w-[90%] mx-auto">
        ...
        <p className="text-gray-600 mb-6">Your Vehicle Loan application has been submitted successfully.</p>
