...
const EnquiryAdminPage = () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        const transformedData: Enquiry[] = response.contactus.map((item, index) => ({
          ...
          status: (item.status as 'Pending' | 'Closed') || 'Pending',
  ...
  // Filter enquiries based on active tab and search
  ...
  // Tabs configuration
  const tabs = ['All', 'Pending', 'Closed'] as const;
...
const updateStatus = async (id: number, newStatus: Enquiry['status']) => {
  ...
  const originalStatus = enquiryToUpdate.status;
  ...
  try {
    ...
    const response = await AdminService.updateContactStatus(
      enquiryToUpdate.enquiry_id, 
      newStatus
    );

    if (!response.success) {
      throw new Error(response.message || 'Failed to update status on server');
    }
    ...
    setEnquiries(prev => 
      prev.map(enquiry => 
        enquiry.id === id ? { ...enquiry, status: originalStatus } : enquiry
      )
    );
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex-1 relative">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search enquiries by ID, type, name, email, phone, message, or status..."
                  value={searchTerm}
            ...
            <div className="text-sm text-gray-600">
              <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                <div className="flex items-center space-x-2">
                  <span>
                    Showing <span className="font-bold">{filteredEnquiries.length}</span> of{' '}
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              {sortedEnquiries.map((enquiry) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900 flex items-center">
                      {renderIcon(PhoneIcon, "", { className: "h-4 w-4 mr-2 text-gray-400" })}
                      {enquiry.phone}
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-sm text-gray-900">
                      <div className="truncate max-w-xs" title={enquiry.message}>
                        {enquiry.message}
                      </div>
                    </div>
                  </td>
          ...
          {sortedEnquiries.length > 0 && (
            <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
              ...
              <div className="flex space-x-2">
                ...
                <button className="px-3 py-1 border border-gray-300 rounded text-gray-700 bg-blue-50 text-blue-600">
                  1
                </button>
                <button className="px-3 py-1 border border-gray-300 rounded text-gray-700 hover:bg-gray-50">
                  Next
                  ...
                  <div>
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      ...
                      <p className="text-sm text-gray-600 mt-2">
                        <span className="font-medium">Enquiry Type:</span> {selectedEnquiry.enquiry_type}
                      </p>
                    </div>
                  </div>
