...
const ConsumerDetailedLeads: FC = () => {
  ...
  const fetchLeads = async () => {
    try {
      setLoading(true);
      const response = await RmService.getConsumerDetailedLeads();
      ...
      if (response && response.success && Array.isArray(response.leads)) {
        const mappedLeads: Lead[] = response.leads.map((item: any) => {
          ...
          return {
            ...
            formData: item.form_data || item, // Use whole item if form_data is missing
  ...
  const loadDocuments = async (leadId: string) => {
    ...
    try {
      // Note: Using leadId directly as per your rmService structure
      const response = await (RmService as any).getLeadDocuments(leadId);
  ...
  return (
    <div className="p-4 md:p-6 bg-gray-50 min-h-screen font-sans">
      <div className="max-w-[1800px] mx-auto">
                ...
                {currentLeads.length > 0 ? (
                  currentLeads.map((lead) => (
                    <tr key={lead.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-4 text-sm text-gray-900 font-meadium">{lead.clientName}</td>
        ...
        {/* DOCUMENT MODAL - Matches Reference Design Exactly */}
        {viewingDocs && (
        ...
        {/* FORM DATA MODAL - Matches Reference Design Exactly */}
        {viewingForm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
