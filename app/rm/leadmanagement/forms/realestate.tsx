...
export default function RealEstateForm({ onClose }: { onClose: () => void }) {
  ...
  const submitForm = async (e: any) => {
    ...
    if (!dsaId.trim()) {
      setDsaError("Select a DSA partner");
      setError(true);
      return;
    }
          ...
          <form onSubmit={submitForm} className="p-4 sm:p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              ...
              <Input 
                ...
                onChange={(e: any) => handleChange("capitalAvailable", e.target.value)}
              />
              ...
              <Input 
                ...
                onChange={(e: any) => handleChange("appointmentDate", e.target.value)}
              />

              {/* Real Estate Type Select */}
              <Select
                label="Type of Real Estate"
                options={["Investment", "Residential", "Commercial"]}
                value={formData.realestateType}
                onChange={(value: string) => handleChange("realestateType", value)}
              />
