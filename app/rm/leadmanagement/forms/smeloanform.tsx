...
const DOC_REGISTRY: Record<string, { key: string; label: string; multiple: boolean }> = {
  ...
  "Address Proof (Rent Agmt/Light Bill)": { key: "ADDRESS_PROOF", label: "Address Proof (Rent Agmt/Light Bill)", multiple: false },
  "GST Registration Certificate": { key: "GST_CERTIFICATE", label: "GST Registration Certificate", multiple: false },
  ...
  "Constitution Doc (Partnership/MOA)": { key: "CONSTITUTION_DOC", label: "Constitution Doc (Partnership/MOA)", multiple: true },
...
const BASE_DOCS = ["Aadhar Card", "PAN Card", "Address Proof (Rent Agmt/Light Bill)", "GST Registration Certificate", "Udyam Aadhar", "Shop Act Licence", "1 Year Banking Statement", "ITR 3 Years", "Constitution Doc (Partnership/MOA)", "Photograph"];
...
export default function SMELoanForm({ onClose }: { onClose: () => void }) {
  ...
  const handleCreateLead = async () => {
    ...
    try {
      const payload = {
        department: "Loan", product_type: "SME Loan", sub_category: "SME Loan",
        ...
        form_data: {
          dob: form.dob, location: form.location, loanAmount: form.loanAmount,
          hasOtherLoan: form.hasOtherLoan, otherLoanAmount: form.otherLoanAmount || "0"
        }
      };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-2 sm:p-4 text-gray-700">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl mx-auto h-[95vh] sm:h-[90vh] flex flex-col relative overflow-hidden">
        ...
        <div className="flex justify-between items-center border-b px-4 sm:px-6 py-3 sm:py-4 bg-white">
          <div>
            <h2 className="text-lg sm:text-xl font-semibold text-[#1CADA3]">SME Loan Application</h2>
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
                <div className="mb-6 p-4 bg-blue-50 border border-blue-100 rounded-lg flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-blue-700">Lead ID: <span className="font-mono font-bold">{leadId}</span>. Upload documents to finish.</p>
                    </div>
                </div>
    ...
    return (
      <div className="absolute inset-0 z-[60] flex items-center justify-center bg-black/60 rounded-xl">
        <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-2xl text-center max-w-sm w-[90%] animate-in zoom-in duration-300">
          ...
          <p className="text-gray-600 mb-6">Your SME Loan application and documents have been processed successfully.</p>
