    ...
    try {
      if (wishlistedIds.has(ncd.id)) {
        ...
        if (response.success) {
          ...
          // Notify other components (wishlist page) to refresh
            ...
            <div className="w-full sm:w-auto mt-4 sm:mt-0">
              <div className="p-1 bg-slate-100/80 backdrop-blur-sm rounded-2xl sm:rounded-full flex flex-col sm:flex-row sm:items-center gap-1 relative shadow-inner border border-slate-200/50">
                ...
                <button
                  ...
                  className={`w-full sm:w-auto px-4 md:px-5 py-2.5 md:py-2 rounded-xl sm:rounded-full text-[10px] md:text-xs font-black uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all ${mainTab === 'explore'
                ...
                <button
                  ...
                  className={`relative w-full sm:w-auto px-4 md:px-5 py-2.5 md:py-2 rounded-xl sm:rounded-full text-[10px] md:text-xs font-black uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all ${mainTab === 'investments'
                      ? "text-white"
                      : "text-slate-500 hover:text-slate-700"
                    }`}
                >
                    ...
                    <div className="flex p-1 bg-slate-50 rounded-2xl border border-slate-100 w-full sm:w-auto">
                      {(["Open", "Upcoming", "Closed"] as const).map((tab) => (
                        <button
                          ...
                          className={`flex-1 sm:flex-none px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-[0.1em] sm:tracking-[0.15em] transition-all whitespace-nowrap ${ncdStatusTab === tab
                        ...
                        <div className="px-6 py-4 relative">
                          <div className="absolute top-4 right-6 flex items-center gap-2">
                            <button
                              ...
                              className={`p-2 rounded-xl border transition-all ${wishlistedIds.has(ncd.id)
                            ...
                            <span className={`text-[9px] font-black uppercase tracking-[0.2em] px-3 py-1.5 rounded-lg border ${ncdStatusTab === "Open" ? "bg-emerald-50 border-emerald-100 text-emerald-600"
