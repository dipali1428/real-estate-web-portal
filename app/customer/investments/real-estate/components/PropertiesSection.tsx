...
import {
    ...
    ChevronLeft, Layout, Zap, Info, Plus, Minus, IndianRupee, Bookmark
} from 'lucide-react';
import customerService from '../../../../services/customerService';
import toast from 'react-hot-toast';
import RealEstateForm from '../../../../dashboard/leadmanagement/forms/realestate';
...
const PropertiesSection = ({ onPropertySelect, showOnlyLive = false, isDashboard = false }: PropertiesSectionProps) => {
    ...
    const [wishlistItems, setWishlistItems] = useState<{product_id: number, wishlist_id: number}[]>([]);
    ...
    const fetchWishlist = async () => {
        ...
        try {
            ...
            if (response.success && response.data) {
                ...
                const reWishlist = dataArray.filter((item: any) => item.product_type === 'real-estate' || item.product_type === 'real_estate');
                setWishlistItems(reWishlist.map((item: any) => ({
    ...
    const handleToggleWishlist = async (property: TransformedProperty) => {
        ...
        const wishlistItem = wishlistItems.find(i => i.product_id === property.id);

        try {
            if (wishlistItem) {
                const response = await customerService.removeFromWishlist(wishlistItem.wishlist_id);
                if (response.success) {
                    setWishlistItems(prev => prev.filter(i => i.product_id !== property.id));
                ...
                const payload = {
                    ...
                    logo_url: property.image,
                    min_investment: property.min_contribution,
                    yield_percentage: property.yield_percentage
                };
                const response = await customerService.addToWishlist(payload);
                if (response.success) {
                    const returnedItem = Array.isArray(response.data) ? response.data[0] : response.data;
                    setWishlistItems(prev => [...prev, {
                        product_id: property.id,
                        wishlist_id: returnedItem?.id || Date.now()
                    }]);
    ...
    const transformedProperties: TransformedProperty[] = useMemo(() => {
        const apiTransformed = apiProperties.map(prop => ({
            id: prop.id, title: prop.project_name, developer_name: prop.developer_name, location: prop.location,
            ...
            area: prop.total_area || 500, rera_reg_no: prop.rera_id || 'RERA-Pending',
            min_contribution: parseInt(prop.min_investment), yield_percentage: parseFloat(prop.yield_percentage),
            irr_percentage: prop.irr_percentage ? parseFloat(prop.irr_percentage) : parseFloat(prop.yield_percentage) * 1.2,
            image: prop.image_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(prop.project_name)}&size=400&background=2076C7&color=fff&bold=true`, status: prop.status === 'ACTIVE' ? 'live' : 'closed'
        }));

        // Convert static properties to match TransformedProperty interface
        const staticTransformed = staticProperties.map(p => ({
            id: p.id, title: p.title, developer_name: p.developer, location: p.location,
            type: p.type, price: p.price, area: p.area, rera_reg_no: p.rera_reg_no,
            min_contribution: p.min_contribution, yield_percentage: p.yield_percentage,
            irr_percentage: p.irr_percentage, image: p.image, status: p.status,
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            // Smoothly scroll by 1.5px (adjusted for better feel)
            scrollContainer.scrollLeft += 1.5;

            // Seamless loop: check if we've scrolled past the first set of items
    ...
    return (
        <div id="properties" className="animate-fade-in font-sans">
                                    ...
                                    <div key={`${property.status}-${property.id}-${idx}`} className="bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 flex flex-col h-full border border-slate-100 group">
                                        <div className="relative">
                                            ...
                                            <button 
                                                ...
                                                className="absolute top-4 right-4 p-2.5 bg-white/90 backdrop-blur-md rounded-full shadow-md text-slate-400 hover:text-[#2076C7] hover:scale-105 active:scale-95 transition-all z-10"
                                                title={wishlistItems.some(i => i.product_id === property.id) ? "Remove from wishlist" : "Add to wishlist"}
                                            >
                                                <Bookmark className={`w-4 h-4 transition-colors ${wishlistItems.some(i => i.product_id === property.id) ? 'fill-[#2076C7] text-[#2076C7]' : ''}`} />
                                            </button>
                                        </div>
                                        <div className="p-5 flex-1 flex flex-col">
                                            ...
                                            <div className="mt-auto flex flex-col gap-3 border-t border-slate-100 pt-4">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <button onClick={() => setShowRealEstateForm(true)} className="flex-1 py-2.5 text-xs font-black rounded-xl bg-gradient-to-r from-[#2076C7] to-[#1CADA3] text-white shadow-md hover:shadow-lg transition-all">Apply Now</button>
                                                    <button onClick={() => onPropertySelect && onPropertySelect(String(property.id))} className="border-2 border-[#2076C7] text-[#2076C7] px-4 py-2.5 text-xs rounded-xl shrink-0 font-extrabold hover:bg-blue-50 transition-all hover:scale-105 active:scale-95">Details</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {closedProperties.map((property, idx) => (
                                        <div key={`${property.status}-${property.id}-${idx}`} className="bg-white rounded-xl overflow-hidden shadow-sm flex flex-col border border-slate-100 opacity-90 grayscale-[0.5] hover:grayscale-0 transition-all duration-500 w-full sm:w-[calc(50%-1rem)] lg:w-[calc(25%-0.75rem)] shrink-0">
                                            <div className="relative">
                                                ...
                                                <button 
                                                    onClick={(e) => { e.stopPropagation(); handleToggleWishlist(property); }}
                                                    className="absolute top-3 right-3 p-2 bg-white/90 backdrop-blur-md rounded-full shadow-md text-slate-400 hover:text-[#2076C7] hover:scale-105 active:scale-95 transition-all z-10"
                                                    title={wishlistItems.some(i => i.product_id === property.id) ? "Remove from wishlist" : "Add to wishlist"}
                                                >
                                                    <Bookmark className={`w-3.5 h-3.5 transition-colors ${wishlistItems.some(i => i.product_id === property.id) ? 'fill-[#2076C7] text-[#2076C7]' : ''}`} />
                                                </button>
                                            </div>
                        ...
                        <div className="mb-24 px-2 sm:px-6 lg:px-8 relative overflow-hidden">
                            <div className="max-w-6xl mx-auto">
                                ...
                                <div className="flex flex-col lg:flex-row items-stretch gap-8 relative">
                                    ...
                                    <motion.div
                                        whileHover={{ y: -5 }}
                                        className="flex-1 bg-white rounded-[2rem] p-5 md:p-8 border border-slate-200 relative overflow-hidden group"
                                    >
                                    ...
                                    <motion.div
                                        whileHover={{ y: -10 }}
                                        className="flex-1 bg-blue-50/50 rounded-[2rem] p-5 md:p-8 border-2 border-[#1CADA3]/20 shadow-[0_30px_70px_rgba(32,118,199,0.08)] relative overflow-hidden group"
                                    >
                                        ...
                                        <div className="relative z-10">
                                            ...
                                            <div className="mt-12 pt-8 border-t border-slate-100 text-center">
                                                <button
                                                    ...
                                                    className="w-full py-4 bg-gradient-to-r from-[#2076C7] to-[#1CADA3] rounded-2xl text-white font-black text-xs uppercase tracking-[0.3em] shadow-lg hover:shadow-[#2076C7]/20 transition-all flex items-center justify-center gap-3 group"
                                                >
            ...
            {showRealEstateForm && (
                <RealEstateForm onClose={() => setShowRealEstateForm(false)} />
            )}
        </div>
    );
};
