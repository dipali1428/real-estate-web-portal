...
const StatCard = ({ title, value, icon, color, subtitle, trend }: StatCardProps) => (
  <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 hover:shadow-md transition-all">
      ...
      <div className="flex-1 p-4 sm:p-6 bg-[#F8FAFC] min-h-screen">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center px-4">
            ...
            <p className="text-gray-600 mt-6 font-medium text-sm sm:text-base">Loading reports...</p>
          </div>
        </div>
      </div>
    );
      ...
      <section className="animate-fade-in">
        ...
        >
          <div className="flex justify-between items-start flex-wrap gap-3 sm:gap-4">
            <div>
              <h2 className="text-lg sm:text-xl md:text-2xl font-bold mb-1 sm:mb-2 flex items-center gap-1 sm:gap-2">
        ...
        {activeTab === 'overview' && overview && (
          ...
          >
            ...
            {/* Investment Categories - Responsive Grid */}
            <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
                ...
                <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-lg sm:rounded-xl p-3 sm:p-4 border border-blue-100">
                  ...
                  <div className="text-sm sm:text-lg font-bold text-gray-800">{formatLargeNumber(overview.unlisted_shares.total_value)}</div>
                ...
                <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg sm:rounded-xl p-3 sm:p-4 border border-purple-100">
                  ...
                  <div className="text-sm sm:text-lg font-bold text-gray-800">{formatLargeNumber(overview.mutual_funds.total_value)}</div>
                ...
                <div className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-lg sm:rounded-xl p-3 sm:p-4 border border-orange-100">
                  ...
                  <div className="text-sm sm:text-lg font-bold text-gray-800">{formatLargeNumber(overview.bonds.total_value)}</div>
                ...
                <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-lg sm:rounded-xl p-3 sm:p-4 border border-emerald-100">
                  ...
                  <div className="text-sm sm:text-lg font-bold text-gray-800">{formatLargeNumber(overview.fd.total_value)}</div>
                ...
                <div className="bg-gradient-to-br from-rose-50 to-red-50 rounded-lg sm:rounded-xl p-3 sm:p-4 border border-rose-100">
                  ...
                  <div className="text-sm sm:text-lg font-bold text-gray-800">{formatLargeNumber(overview.ipo.total_value)}</div>
                  <div className="text-[10px] sm:text-xs text-gray-500 mt-0.5 sm:mt-1">{overview.ipo.total_ipo} Applications</div>
                </div>
              </div>
            </div>

            {/* Unlisted Shares Details - Responsive Grid */}
              ...
              <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                  <div className="text-center p-2 sm:p-3 bg-gray-50 rounded-lg sm:rounded-xl">
                    ...
                    <p className="text-sm sm:text-xl font-bold text-gray-800">{overview.unlisted_shares.total_holdings}</p>
                  </div>
                  <div className="text-center p-2 sm:p-3 bg-gray-50 rounded-lg sm:rounded-xl">
                    <p className="text-[10px] sm:text-xs text-gray-500">Total Value</p>
                    <p className="text-sm sm:text-xl font-bold text-gray-800">{formatLargeNumber(overview.unlisted_shares.total_value)}</p>
                  </div>
                  <div className="text-center p-2 sm:p-3 bg-gray-50 rounded-lg sm:rounded-xl">
                    <p className="text-[10px] sm:text-xs text-gray-500">Total Transactions</p>
                    <p className="text-sm sm:text-xl font-bold text-gray-800">{overview.unlisted_shares.total_transactions}</p>
                  </div>
                  <div className="text-center p-2 sm:p-3 bg-gray-50 rounded-lg sm:rounded-xl">
                    <p className="text-[10px] sm:text-xs text-gray-500">Pending Transactions</p>
                    <p className="text-sm sm:text-xl font-bold text-yellow-600">{overview.unlisted_shares.pending_transactions}</p>
                  </div>
                </div>
              </div>
            )}
        ...
        {activeTab === 'products' && (
          <motion.div
            ...
            className="space-y-4 sm:space-y-6"
          >
            {/* Product Summary Table - Horizontal Scroll on Mobile */}
            <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100">
              <h3 className="text-base sm:text-lg font-semibold text-gray-800 mb-3 sm:mb-4 flex items-center gap-2">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {reportData.productSummary.map((item, idx) => (
                        <tr key={idx} className="hover:bg-gray-50 transition-colors">
                          <td className="py-2 sm:py-3 px-3 sm:px-4 font-medium text-gray-800 text-xs sm:text-sm">{item.product_type}</td>
                          <td className="py-2 sm:py-3 px-3 sm:px-4 text-right text-gray-600 text-xs sm:text-sm">{item.total_orders}</td>
                          <td className="py-2 sm:py-3 px-3 sm:px-4 text-right font-semibold text-gray-800 text-xs sm:text-sm">{formatCurrency(item.total_amount)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {/* Bar Chart Visualization - Responsive */}
        ...
        {activeTab === 'distribution' && (
          ...
          >
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-3">
                            {reportData.portfolioDistribution.map((item, idx) => (
                                <div key={idx} className="flex items-center gap-1 sm:gap-2 p-1.5 sm:p-2 rounded-lg bg-gray-50 hover:bg-gray-100 transition-all">
                                    <div className="w-2 h-2 sm:w-3 sm:h-3 rounded-full" style={{ backgroundColor: item.color || `#${Math.floor(Math.random()*16777215).toString(16)}` }}></div>
            ...
            {/* Recent Investments - Horizontal Scroll on Mobile */}
            <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100">
              <h3 className="text-base sm:text-lg font-semibold text-gray-800 mb-3 sm:mb-4 flex items-center gap-2">
                <ClockIcon className="w-4 h-4 sm:w-5 sm:h-5 text-[#2076C7]" />
                Recent Investments
              </h3>
                  ...
                  <table className="w-full min-w-[400px]">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {reportData.recentInvestments.map((item, idx) => (
                        <tr key={idx} className="hover:bg-gray-50 transition-colors">
                          <td className="py-2 sm:py-3 px-3 sm:px-4 text-[10px] sm:text-sm text-gray-600">{item.date || 'N/A'}</td>
                          <td className="py-2 sm:py-3 px-3 sm:px-4 font-medium text-gray-800 text-[10px] sm:text-sm">{item.product_name || item.name || 'Investment'}</td>
                          <td className="py-2 sm:py-3 px-3 sm:px-4 text-right font-semibold text-gray-800 text-[10px] sm:text-sm">{formatCurrency(item.amount || 0)}</td>
                          <td className="py-2 sm:py-3 px-3 sm:px-4 text-center">
                            <span className={`inline-flex items-center gap-0.5 sm:gap-1 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full text-[8px] sm:text-xs font-medium ${
                              item.status === 'completed' ? 'bg-green-100 text-green-700' :
                              item.status === 'pending' ? 'bg-yellow-100 text-yellow-700' :
                              'bg-gray-100 text-gray-700'
                            }`}>
                              {item.status === 'completed' && <CheckCircle size={8} />}
                              {item.status === 'pending' && <ClockIcon size={8} />}
                              {item.status || 'N/A'}
                            </span>
                          </td>
                        </tr>
