...
import LASTypesSection from './components/LASTypesSection';
import { LASCalculator } from './components/LASCalculator';

export default function LoanAgainstSecuritiesDashboard() {
    const [activeTab, setActiveTab] = useState<'marketable' | 'fixed' | 'calculator'>('marketable');
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                        ...
                        <div className="min-w-0">
                            <div className="flex flex-col sm:flex-row items-center gap-3 mb-1">
                                <h2 className="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight leading-tight">
                                    {activeTab === 'marketable' ? 'Marketable Securities' : activeTab === 'fixed' ? 'Fixed Income & Bonds' : 'LAS Credit Architect'}
                                </h2>
                                <span className="px-2.5 py-1 bg-emerald-100 text-emerald-700 text-[10px] font-black rounded-full border border-emerald-200 whitespace-nowrap uppercase tracking-wider">
                                    {activeTab === 'calculator' ? 'Risk Analysis' : 'Live Plans'}
                                </span>
                            </div>
                            <p className="text-sm text-slate-500 font-medium flex items-center justify-center sm:justify-start gap-2">
                                <span>
                                    {activeTab === 'calculator'
                                        ? 'Analyze margin risk and top-up eligibility instantly'
                                        : 'Unlock capital from your existing assets without liquidation'}
                                </span>
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="w-full sm:w-auto mt-4 sm:mt-0">
                        <div className="grid grid-cols-2 sm:flex sm:flex-row p-1.5 bg-slate-100/80 backdrop-blur-sm rounded-2xl sm:rounded-full gap-1.5 relative shadow-inner border border-slate-200/50">
                            {[
                                { id: 'marketable', label: 'Marketable', icon: IconChartLine },
                                { id: 'fixed', label: 'Fixed Income', icon: IconBriefcase },
                ...
                <AnimatePresence mode="wait">
                    {activeTab === 'calculator' ? (
                        ...
                        <motion.div
                            ...
                            className="w-full"
                        >
                            <LASTypesSection activeTab={activeTab as any} />
                        </motion.div>
                    )}
                </AnimatePresence>
            </div>
            ...
            <div className="mt-12">
                <div className="bg-slate-50 border border-slate-100 rounded-2xl p-6 shadow-sm">
                    <p className="text-sm text-slate-500 text-center leading-relaxed font-medium">
                        ...
                        Loan Against Securities (LAS) is subject to market risks. LTV and interest rates are indicative and subject to the policies of the respective banks or financial institutions. Portfolio valuation and eligibility are verified at the time of pledge.
                    </p>
                </div>
            </div>
        </div>
