...
export default function LifeInsuranceHeader() {
  ...
  return (
    <motion.div
      ...
      className="mb-6 md:mb-8 font-sans"
    >
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        ...
        <div className="w-full xl:w-auto mt-4 xl:mt-0">
          <div className="p-1.5 bg-slate-100/80 backdrop-blur-sm rounded-2xl sm:rounded-full grid grid-cols-2 sm:flex sm:flex-row sm:items-center gap-1.5 sm:gap-1 relative shadow-inner border border-slate-200/50">
            {navItems.map((item) => (
              <HeaderButton
                key={item.label}
                label={item.label}
