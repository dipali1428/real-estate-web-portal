...
export default function MyApplicationsPage() {
  ...
  return (
    <div className="flex-1 p-4 md:p-10 bg-slate-50 font-sans min-h-screen">
      {/* ─── Header ─── */}
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8 pb-6 border-b border-slate-200">
        <div>
          <button 
            onClick={() => router.push("/customer/loans/personal-loan")}
            className="flex items-center gap-2 text-slate-500 hover:text-[#2076C7] text-[10px] font-black uppercase tracking-widest mb-4 transition-colors"
          >
          ...
          <div className="flex items-center gap-2 text-[10px] text-[#2076C7] font-black uppercase tracking-[0.2em] mb-3 bg-blue-50/50 w-fit px-3 py-1 rounded-md border border-blue-100">
            <Clock size={12} className="animate-pulse" /> Application Center
          </div>
          <h1 className="text-2xl md:text-3xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-[#2076C7] to-[#1CADA3]">
            My Applications
          </h1>
          <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mt-1">
            Track and manage your personal loan applications
          </p>
        </div>
        <div className="flex items-center gap-3">
          <div className="bg-white border border-slate-200 px-4 py-3 rounded-xl flex items-center gap-3 shadow-sm md:w-64">
             <Search size={16} className="text-slate-400" />
             <input type="text" placeholder="Search ID..." className="bg-transparent border-none outline-none text-xs font-medium w-full text-slate-700 placeholder-slate-400" />
          </div>
        </div>
      </div>
        ...
        {APPLICATIONS.length === 0 ? (
          <div className="space-y-6">
            ...
            >
              ...
              {/* LIVE PREVIEW Tracker showing what will be tracked */}
              <div className="relative z-10 w-full max-w-4xl mx-auto my-10 text-left">
                <div className="flex items-center gap-2 mb-3 px-2">
                   <Eye size={16} className="text-slate-400" />
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] relative top-0.5">Live Preview: Application Tracking Experience</p>
                </div>
                
                {/* Dummy Application Card */}
                <div className="bg-white/80 backdrop-blur-md rounded-[2rem] border border-slate-200/60 shadow-sm overflow-hidden pointer-events-none opacity-90 transition-all hover:opacity-100">
                  <div className="p-5 md:p-6 flex flex-col md:flex-row md:items-center gap-4 border-b border-slate-100/50">
                    <div className="flex items-center gap-4 flex-1">
                      <div className="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center shrink-0 shadow-sm border border-slate-100 grayscale">
                        <Landmark size={20} className="text-slate-400" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-1">
                          <h3 className="text-sm font-black text-slate-500 uppercase tracking-tight">Banking Partner (Sample)</h3>
                          <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 border border-slate-100 px-2 py-0.5 rounded-md shrink-0">
                            Personal Loan
                          </span>
                        </div>
                        <div className="flex items-center gap-3 text-slate-400">
                          <span className="text-[10px] font-bold">APP-000000</span>
                          <span className="text-[10px]">•</span>
                          <span className="text-[10px] font-bold">₹0 Sanctioned</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center md:items-end gap-4 shrink-0">
                      <span className="text-[9px] font-black uppercase tracking-[0.1em] px-3 py-1.5 rounded-lg border bg-amber-50/50 text-amber-500/70 border-amber-100/50">
                        In Progress (Demo)
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-5 md:p-6 bg-slate-50/30">
                    <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-1">
                      {STATUS_STEPS.map((step, i) => {
                        const isCompleted = i <= 2; // Demo showing progress up to Credit Check
                        const isCurrent = i === 2;
                        return (
                          <React.Fragment key={step.key}>
                            <div className="flex flex-row md:flex-col items-center gap-3 md:gap-2">
                              <div className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center text-xs transition-all ${
                                ...
                                {isCompleted && !isCurrent ? <CheckCircle2 size={16} /> : <step.icon size={16} />}
                              </div>
                            ...
                            {i < STATUS_STEPS.length - 1 && (
                              <div className={`hidden md:block flex-1 h-[2px] rounded-full opacity-50 -mt-5 ${
                                i < 2 ? "bg-slate-300" : "bg-slate-200"
                              }`} />
                            )}
                          </React.Fragment>
                        );
                    ...
                    <div className="mt-5 bg-white rounded-xl p-3 border border-slate-100 flex items-start gap-3 opacity-70">
                      <Clock size={16} className="text-slate-400 shrink-0 mt-0.5" />
                      <p className="text-[10px] text-slate-500 font-medium">
                        <span className="font-black text-slate-600 mr-2">Sample Status:</span>
                        This is a preview. Your actual tracking timeline will update automatically upon application.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { icon: Zap, title: "Instant Approval", desc: "Get sanctioned in under 24 hours with our AI-driven risk assessment model.", color: "text-amber-500", bg: "bg-amber-50", border: "border-amber-100" },
                { icon: Shield, title: "Bank-Grade Security", desc: "Your documents and financial data are encrypted natively inside our vault.", color: "text-emerald-500", bg: "bg-emerald-50", border: "border-emerald-100" },
                { icon: TrendingUp, title: "Unbeatable Rates", desc: "Instantly compare multiple tailored plans side-by-side to find your match.", color: "text-[#2076C7]", bg: "bg-blue-50", border: "border-blue-100" }
              ].map((b, i) => (
                <motion.div 
                  key={i}
                  initial={{ opacity: 0, y: 20 }}
                ...
                >
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 ${b.bg} ${b.border} border transition-transform group-hover:scale-110`}>
                    <b.icon size={24} className={b.color} />
                  </div>
                  <h3 className="text-sm font-black text-slate-800 uppercase tracking-tight mb-3">{b.title}</h3>
                                    ...
                                    <div className="bg-gradient-to-r from-blue-50 to-teal-50 rounded-2xl p-6 border border-blue-100">
                                      ...
                                      <div className="flex justify-between mt-4">
                                        ...
                                        <div className="text-right">
                                          ...
                                          <p className="text-base font-black text-amber-600">₹{(app.totalInterest || 0).toLocaleString("en-IN")}</p>
                                        </div>
                                      </div>
                                    </div>
