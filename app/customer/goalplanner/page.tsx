...
import {
    Target, Plus, TrendingUp, Wallet, CheckCircle2, AlertCircle,
    Home, GraduationCap, Car, Plane, Briefcase,
    IndianRupee, X, Save, Edit2, Calendar, Sparkles,
...
export default function GoalPlanner() {
    ...
    const getGoalIcon = (goalName: string) => {
        ...
        if (name.includes('retire')) return Briefcase;
    ...
    if (loading && !goals.length) {
        return (
            <div className="min-h-screen flex items-center justify-center">
                <Loader2 className="w-12 h-12 text-[#2076C7] animate-spin" />
            </div>
        );
    }
    ...
    return (
        <>
            ...
            <div className="flex-1 p-4 sm:p-6 bg-[#F8FAFC] min-h-screen font-sans">
                ...
                {/* HEADER SECTION - AS PER YOUR SCREENSHOT */}
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="relative bg-gradient-to-r from-[#2076C7] to-[#1CADA3] rounded-2xl p-6 mb-6 text-white shadow-lg">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            ...
                            <p className="text-white/80">Plan, track, and achieve your financial goals with ease.</p>
                        </div>
                ...
                <div className="w-full">
                    ...
                    {/* RESPONSIVE TAB BUTTONS - INTEGRATED FROM REFERENCE */}
                        ...
                        <div className="p-1 bg-slate-100/80 backdrop-blur-sm rounded-xl md:rounded-full flex flex-col md:flex-row items-stretch md:items-center gap-1 relative shadow-inner border border-slate-200/50 w-full md:w-auto">
                            {[
                                ...
                                <button
                                    ...
                                    className={`relative px-5 py-2.5 md:py-2 rounded-lg md:rounded-full text-[10px] md:text-xs font-black uppercase tracking-widest transition-all duration-300 z-10 flex items-center justify-center md:justify-start gap-2 shrink-0 ${selectedTab === tab.id ? 'text-white' : 'text-slate-500 hover:text-slate-700'}`}
                                >
                                    {selectedTab === tab.id && (
                                        <motion.div
                                            layoutId="activeTabGoalPlanner"
                                    ...
                                    {tab.count !== undefined && tab.count > 0 && (
                                        <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-bold ${selectedTab === tab.id ? 'bg-white/20' : 'bg-slate-200 text-slate-600'}`}>
                                            {tab.count}
                                        </span>
                                    )}
                                </button>
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        >
                                ...
                                <>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                                        ...
                                        <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
                                            ...
                                            <p className="text-3xl font-bold text-emerald-600">{formatLargeCurrency(stats.totalTarget)}</p>
                                        ...
                                        <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
                                            ...
                                            <p className="text-3xl font-bold text-amber-600">{formatLargeCurrency(stats.totalCurrent)}</p>
                                        ...
                                        <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
                                            ...
                                            <p className="text-3xl font-bold text-purple-600">{stats.avgReturn}%</p>
                                    ...
                                    {goals.length === 0 ? (
                                        <div className="bg-white rounded-3xl p-16 text-center border border-gray-200">
                                            ...
                                            <button onClick={() => setSelectedTab('create')} className="px-8 py-4 bg-gradient-to-r from-[#2076C7] to-[#1CADA3] text-white font-medium rounded-xl shadow-lg hover:shadow-xl transition-all">
                            ...
                            {/* OTHER TABS LOGIC - AS PER ORIGINAL */}
                            {(selectedTab === 'active' || selectedTab === 'completed') && (
                                <div className="bg-white rounded-2xl shadow-lg border border-gray-200">
                                    <div className="p-6 border-b border-gray-200"><h2 className="text-xl font-bold text-gray-900">{selectedTab === 'active' ? 'My Goals' : 'Completed Goals'}</h2></div>
                                        ...
                                        {goals.filter(goal => selectedTab === 'active' ? (progress[goal.id] || 0) < 100 : (progress[goal.id] || 0) >= 100).map(goal => (
                                            <div key={goal.id} className="p-6 flex items-center justify-between">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-12 h-12 rounded-xl bg-blue-100 flex items-center justify-center text-blue-600">{React.createElement(getGoalIcon(goal.goal_name), { size: 24 })}</div>
                                                ...
                                                <div className="w-48"><div className="h-2 bg-gray-100 rounded-full"><div style={{ width: `${progress[goal.id] || 0}%` }} className="h-full bg-blue-600 rounded-full" /></div></div>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                ...
                {/* MODAL: VIEW DETAILS - AS PER SCREENSHOT */}
