  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // const incentivesRes = await DashboardService.getIncentives();
        // setIncentiveData(incentivesRes.incentives || []);

        // // Fetch summary data
        // const summaryRes = await DashboardService.getIncentivesSummary();
      ...
      <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          <div className="bg-white rounded-xl p-4 sm:p-6 shadow-sm border border-slate-200 hover:shadow-md transition-all duration-200">
            <div className="flex items-center justify-between">
              <div className="space-y-2 sm:space-y-3">
                <p className="text-xs sm:text-sm font-medium text-slate-600 uppercase tracking-wide">Total Payouts</p>
                <p className="text-2xl sm:text-3xl font-sans text-slate-800">₹{totalPayouts}</p>
          ...
          <div className="bg-white rounded-xl p-4 sm:p-6 shadow-sm border border-slate-200 hover:shadow-md transition-all duration-200">
            <div className="flex items-center justify-between">
              <div className="space-y-2 sm:space-y-3">
                <p className="text-xs sm:text-sm font-medium text-slate-600 uppercase tracking-wide">Pending Payouts</p>
                <p className="text-2xl sm:text-3xl font-sans text-slate-800">₹{pendingPayouts}</p>
          ...
          <div className="bg-white rounded-xl p-4 sm:p-6 shadow-sm border border-slate-200 hover:shadow-md transition-all duration-200">
            <div className="flex items-center justify-between">
              <div className="space-y-2 sm:space-y-3">
                <p className="text-xs sm:text-sm font-medium text-slate-600 uppercase tracking-wide">YTD Earnings</p>
                <p className="text-2xl sm:text-3xl font-sans text-slate-800">₹{ytdEarnings}</p>
                <div className="flex items-center text-slate-600 text-xs sm:text-sm font-medium">
                  <svg className="w-3 h-3 sm:w-4 sm:h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
              ...
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-slate-600 rounded-xl flex items-center justify-center shrink-0">
                <svg className="w-5 h-5 sm:w-6 sm:h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <div className="px-4 sm:px-6 py-4 border-b border-slate-200">
            <div className="flex flex-col gap-4">
              <div>
                <h2 className="text-lg sm:text-xl font-semibold text-slate-900">Transaction History</h2>
              ...
              <div className="flex flex-wrap gap-2">
                {(['all', 'paid', 'processing', 'pending'] as const).map((tab) => (
                  <button
                    ...
                    className={`px-3 py-2 text-xs sm:text-sm font-medium rounded-md transition-all duration-200 capitalize min-w-16 ${
                      activeTab === tab
              ...
              <tbody className="divide-y divide-slate-200">
                {filteredData.map((item) => (
                  ...
                  >
                    ...
                    {/* Product */}
                    <td className="px-4 sm:px-6 py-4">
                      <div className="text-sm text-slate-900 max-w-[150px] sm:max-w-xs truncate" title={item.product}>
                        {item.product}
                      </div>
                      {item.clientName && (
                        <div className="text-xs text-slate-500 mt-1">
                    ...
                    <td className="px-4 sm:px-6 py-4">
                      <div className="flex items-center space-x-2">
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${STATUS_CONFIG[item.status].color}`}>
                          <span className="mr-1">{STATUS_CONFIG[item.status].icon}</span>
                          <span className="hidden sm:inline">{STATUS_CONFIG[item.status].label}</span>
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="px-4 sm:px-6 py-4">
                      <div className="flex flex-col gap-1">
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${PAYMENT_METHOD_CONFIG[item.paymentMethod].color}`}>
                          <span className="mr-1">{PAYMENT_METHOD_CONFIG[item.paymentMethod].icon}</span>
                          <span className="hidden sm:inline">{item.paymentMethod}</span>
                        </span>
                        <div 
                          className="text-xs text-slate-600 max-w-[120px] truncate sm:hidden"
                    ...
                    <td className="px-4 sm:px-6 py-4">
                      <div className="flex items-center space-x-2">
                        <button 
                          ...
                          className="text-slate-600 hover:text-slate-800 transition-colors duration-200 p-1 rounded"
                          title="View Invoice"
                        >
                        ...
                        <button 
                          ...
                          className="text-blue-600 hover:text-blue-700 transition-colors duration-200 p-1 rounded"
                          title="Download PDF"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="px-4 sm:px-6 py-4 border-t border-slate-200 bg-slate-50">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between text-sm text-slate-600">
              <div className="flex items-center space-x-2 mb-3 sm:mb-0">
                ...
                <span className="font-semibold text-slate-900 text-xs sm:text-sm">{filteredData.length}</span>
                <span className="text-xs sm:text-sm">of</span>
                <span className="font-semibold text-slate-900 text-xs sm:text-sm">{incentiveData.length}</span>
                <span className="text-xs sm:text-sm">transactions</span>
              </div>
              <div className="flex items-center space-x-2">
                <button className="px-3 py-2 text-xs sm:text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors duration-200 disabled:opacity-50">
                  Previous
                </button>
                <button className="px-3 py-2 text-xs sm:text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors duration-200">
                  Next
                </button>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          <div className="bg-blue-50 rounded-xl p-4 sm:p-6 border border-blue-200">
            <div className="flex items-start space-x-3 sm:space-x-4">
              ...
              <div>
                <h3 className="text-base sm:text-lg font-semibold text-blue-900 mb-2">Payout Schedule</h3>
          ...
          <div className="bg-slate-100 rounded-xl p-4 sm:p-6 border border-slate-300">
            <div className="flex items-start space-x-3 sm:space-x-4">
              ...
              <div>
                <h3 className="text-base sm:text-lg font-semibold text-slate-900 mb-2">Invoice Generation</h3>
