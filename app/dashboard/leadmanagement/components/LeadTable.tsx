...
export interface Lead {
  ...
  createdTime: string;
  formData?: any;
  isSelfLogin?: string; // Added for the new field
}
...
const LeadTable: FC<LeadTableProps> = ({ onEdit, onDelete }) => {
    ...
    const fetchLeads = async () => {
      try {
        ...
        if (activeTab === 'referral') {
          ...
          response = await DashboardService.getMyLeads();
        }
          ...
          const mappedLeads: Lead[] = response.data.map((item: any) => {
            ...
            if (activeTab === 'detailed') {
              return {
                ...
                isSelfLogin: item.is_self_login ? "Yes" : "No", // Convert boolean to Yes/No
  ...
  const handleFileUpload = async (e: ChangeEvent<HTMLInputElement>, documentKey: string) => {
    ...
    try {
      const response = await (DashboardService as any).uploadDocument(activeLeadId, formData);
      if (response.success) {
  ...
  return (
    <div className="w-full mt-8">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-4 gap-4">
        <h3 className="text-lg font-semibold text-gray-700">{activeTab === 'referral' ? 'Referral Leads' : 'Detailed Leads'}</h3>
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
          ...
          <div className="relative">
            <input type="text" placeholder={`Search...`} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 w-full sm:w-64 text-sm text-gray-700" />
                      ...
                      <>
                        ...
                        <td className="px-4 py-4 text-center">
                          <div className="flex flex-col gap-1 items-center">
                            <button onClick={() => handleViewDocuments(lead.id)} className="text-[10px] w-full bg-blue-50 text-blue-600 px-2 py-1 rounded border border-blue-100 font-bold hover:bg-blue-600 hover:text-white uppercase transition-all">View Docs</button>
                            <button onClick={() => { setSelectedForm(lead.formData); setViewingForm(true); }} className="text-[10px] w-full bg-gray-50 text-gray-600 px-2 py-1 rounded border border-gray-100 font-bold hover:bg-gray-800 hover:text-white uppercase transition-all">View Form</button>
                          </div>
                        </td>
                      </>
                    )}
                      ...
                      <div className="grid grid-cols-1 gap-2">
                        {documents.uploaded.map((doc, index) => (
                          <div key={index} className="flex items-center justify-between p-3 border border-green-100 rounded-lg bg-green-50/50">
                            ...
                            <a href={doc.file_url} target="_blank" rel="noopener noreferrer" className="text-xs bg-blue-600 text-white px-3 py-1.5 rounded font-bold uppercase">View</a>
                          </div>
                        ))}
                      </div>
