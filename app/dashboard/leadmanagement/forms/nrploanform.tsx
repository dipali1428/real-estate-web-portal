...
const DOC_REGISTRY: Record<string, { key: string; label: string; multiple: boolean }> = {
  ...
  "Business Office Address Proof": { key: "BUSINESS_ADDRESS_PROOF", label: "Business Office Address Proof", multiple: false },
...
export default function NrpLoanForm({ onClose }: { onClose: () => void }) {
  ...
  const validateStep1 = () => {
    ...
    if (isSelfLoginActive) {
      ...
      if (!form.rmContact || form.rmContact.length !== 10) errs.rmContact = "Must be 10 digits";
      ...
      req("loanAmount", "Loan Amount is required");
      req("loanCategory", "Select category");
      ...
      if (form.employmentType === "Other") {
        ...
        if (["Pension", "Rental"].includes(form.otherIncomeSource) && !form.otherIncomeAmount) errs.otherIncomeAmount = "Required";
      }
    }
    setErrors(errs);
  ...
  const handleCreateLead = async () => {
    ...
    try {
      ...
      // Routing logic: Direct success for Self-Login, else Step 2
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-2 sm:p-4 text-gray-700">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl mx-auto h-[95vh] sm:h-[90vh] flex flex-col relative overflow-hidden">
        ...
        <div className="flex justify-between items-center border-b px-4 sm:px-6 py-3 sm:py-4 bg-white">
          <div>
            <h2 className="text-lg sm:text-xl font-semibold text-[#1CADA3]">NRP Loan Application</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              ...
              {isSelfLoginActive ? (
                ...
                <>
                  ...
                  <Field label="Location" placeholder="Enter city" value={form.location} onChange={(v:any)=>handleInputChange("location", v)} error={errors.location} required />
                  <Field label="Loan Category" type="select" options={["Fresh", "Resell", "BT"]} value={form.loanCategory} onChange={(v:any)=>handleInputChange("loanCategory", v)} error={errors.loanCategory} required />
                  <Field label="Loan Amount" placeholder="Desired amount" onlyNumber value={form.loanAmount} onChange={(v:any)=>handleInputChange("loanAmount", v)} error={errors.loanAmount} required />
                  ...
                  {form.employmentType === "Other" && (
                    <>
                      <Field label="Other Income Source" type="select" options={["Pension", "Rental"]} value={form.otherIncomeSource} onChange={(v:any)=>handleInputChange("otherIncomeSource", v)} error={errors.otherIncomeSource} required />
                      {["Pension", "Rental"].includes(form.otherIncomeSource) && <Field label="Approx Amount" placeholder="Enter amount" onlyNumber value={form.otherIncomeAmount} onChange={(v:any)=>handleInputChange("otherIncomeAmount", v)} error={errors.otherIncomeAmount} required />}
                    </>
                  )}
                </>
              )}
