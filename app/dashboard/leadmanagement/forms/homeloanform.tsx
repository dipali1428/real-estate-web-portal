...
export default function HomeLoanForm({ onClose }: { onClose: () => void }) {
  ...
  const handleInputChange = (field: string, value: string) => {
    setForm(prev => ({
      ...prev, [field]: value,
      ...(field === "employmentType" && value !== "Other" ? { otherIncome: "", otherIncomeAmount: "" } : {}),
  ...
  const validate = () => {
    ...
    if (form.employmentType === "Other") {
      if (!form.otherIncome) errs.otherIncome = "Select income source";
      if (["Pension", "Rental"].includes(form.otherIncome) && !form.otherIncomeAmount) errs.otherIncomeAmount = "Income amount is required";
    }
            ...
            {requiredDocs.length > 0 && (
              <div className="col-span-1 md:col-span-2 mt-4">
                <h3 className="text-md font-semibold mb-3 text-[#1CADA3] border-b pb-2">
                  Upload Documents <span className="text-sm font-normal text-gray-500 ml-2">(for {form.employmentType === "Other" ? form.otherIncome : form.employmentType})</span>
                </h3>
