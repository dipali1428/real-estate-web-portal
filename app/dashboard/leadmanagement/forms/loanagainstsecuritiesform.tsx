...
const DOC_REGISTRY: Record<string, { key: string; label: string; multiple: boolean }> = {
  ...
  "Demat Account Statement": { key: "DEMAT_STATEMENT", label: "Demat Account Statement", multiple: true },
  "Portfolio Reports": { key: "PORTFOLIO_REPORT", label: "Portfolio Reports", multiple: true },
...
export default function LoanAgainstSecuritiesForm({ onClose }: { onClose: () => void }) {
  ...
  const validateStep1 = () => {
    ...
    if (!form.hasOtherLoan) errs.hasOtherLoan = "Selection required";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-2 sm:p-4 text-gray-700">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl mx-auto h-[95vh] sm:h-[90vh] flex flex-col relative overflow-hidden">
        ...
        <div className="flex justify-between items-center border-b px-4 sm:px-6 py-3 sm:py-4 bg-white">
          <div>
            <h2 className="text-lg sm:text-xl font-semibold text-[#1CADA3]">LAS Loan Application</h2>
