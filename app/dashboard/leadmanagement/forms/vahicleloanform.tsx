...
const DOC_MAP: Record<string, string[]> = {
  "Salaried Person": [
    ...
    "6 Months Banking Statement", "Address Proof", "Photograph", "Company ID Card"
  ],
  "Self Employed": [
    "Aadhar Card", "PAN Card", "Udyam Registration", "Shop Act Licence", 
    "1 Year Current Bank Statement", "Saving Bank Account Statement", "Address Proof", 
...
export default function VehicleLoanForm({ onClose }: VehicleLoanFormProps) {
  ...
  const requiredDocs = useMemo(() => {
    // Start with Vehicle Quotation (Always required for Vehicle Loan)
    const list = ["Vehicle Quotation"];

    // Add Employment Documents
    if (form.employmentType && DOC_MAP[form.employmentType]) {
      list.push(...DOC_MAP[form.employmentType]);
    }

    // Add Existing Loan Statement if applicable
    if (form.hasOtherLoan === "Yes") {
      list.push("Existing Loan Statement");
    }
  ...
  const validateForm = useCallback((): boolean => {
    ...
    if (!form.vehicleCompany.trim()) newErrors.vehicleCompany = "Vehicle company is required";
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6">
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            ...
            <Field 
              ...
              onChange={(v) => handleInputChange("vehicleType", v)} 
            ...
            <Field 
              ...
              onChange={(v) => handleInputChange("vehicleCompany", v)} 
